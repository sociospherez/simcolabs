...
export default function Header() {
  return (
    <header className="sticky top-0 z-50 border-b border-[var(--border-subtle)] bg-[#050b16]/70 backdrop-blur-xl">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
        <NavLink to="/" className="flex items-center gap-3 no-underline">
          ...
          <div>
            <div className="text-lg font-semibold tracking-tight theme-text-primary">SimCo Labs</div>
        ...
        <nav className="hidden gap-6 md:flex">
          {navigation.map((item) => (
            <NavLink
              ...
              to={item.path}
              className={({ isActive }) =>
                `text-sm no-underline transition ${isActive ? 'text-cyan-300' : 'theme-text-secondary hover:theme-text-primary'}`
              }
            >
              {item.label}
            </NavLink>
